                    ...
                    {chemicals.map((chemical, index) => (
                        <div key={index}>
                            ...
                            >
                                ...
                                {chemlist.map(chem => (
                                    <option key={chem.id} value={chem.name}>{chem.name} {chem.quantity}{chem.quantity_type}</option>
                                ))}
                            </select>
...
const CompanyOrders = () => {
    ...
    return (
        <div className={'mb-10'}>
            ...
            <button className="flex fixed right-3 top-20 items-center justify-center bg-blue-500 text-white rounded-full w-12 h-12" onClick={toggleModal}>
                        ...
                        <select value={maill} onChange={(e) => setMaill(e.target.value)}>
                            ...
                            {maill === "" && (
                                orderStack1.map(order => (
                                    <option key={order.id} value={order.company_email}>{order.company_email}</option>
                                ))
                            )}
                        </select>
                    ) : (
